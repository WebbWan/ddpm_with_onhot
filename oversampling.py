...
def evaloversampling(y_test, y_predict):
...
def save_result(name, idx, method, accuracy, precision, recall, f1):
    ...
    if not os.path.exists(path="./eval/oversampling_result/{}/{}".format(method, idx)):
        os.mkdir("./eval/oversampling_result/{}/{}".format(method, idx))
    f = open("./eval/oversampling_result/{}/{}/{}.json".format(method, idx, name), 'w')
    f.write(result_json)


def oversamplingSVM(name: str, idx=0):
    method = "SVM"
    x_resample, y_resample, x_test, y_test = oversampling(name, idx=idx)
    clf = SVC().fit(x_resample, y_resample)
    y_predict = clf.predict(x_test)
    accuracy, precision, recall, f1 = evaloversampling(y_test, y_predict)
...
def oversamplingLR(name: str, idx=0):
...
def oversamplingMLP(name: str, idx=0):
    method = "MLP"
    x_resample, y_resample, x_test, y_test = oversampling(name, idx=idx)
...
if __name__ == '__main__':
    for file in os.listdir("./result/raw_data"):
        ...
        for count in range(10):
            ...
            oversamplingSVM(file, count)
            print("-" * len(a))
            oversamplingLR(file, count)
            print("-" * len(a))
            oversamplingMLP(file, count)
            print("=" * len(a))
